...
fn find_existing_uvx() -> Option<String> {
  ...
  #[cfg(target_os = "windows")]
  {
      ...
      // Try finding with where command
      match Command::new("where").arg("uvx").output() {
          Ok(output) if output.status.success() => {
...
pub fn get_nvm_node_paths() -> Result<(String, String), String> {
  ...
  #[cfg(target_os = "windows")]
  {
      ...
      let nvm_root = std::env::var("NVM_HOME").ok()
          .map(std::path::PathBuf::from)
          .or_else(|| {
              dirs::home_dir().map(|p| p.join("AppData").join("Roaming").join("nvm"))
          })
      ...
      if !node_path.exists() {
          return Err(format!("Node.js executable not found at expected path: {}", node_path.display()));
      }

      if !npx_path.exists() {
          return Err(format!("NPX executable not found at expected path: {}", npx_path.display()));
      }
...
fn install_nvm() -> Result<(), String> {
  ...
  #[cfg(target_os = "windows")]
  {
      ...
      let nvm_installer_url = "https://github.com/coreybutler/nvm-windows/releases/download/1.1.11/nvm-setup.exe";
